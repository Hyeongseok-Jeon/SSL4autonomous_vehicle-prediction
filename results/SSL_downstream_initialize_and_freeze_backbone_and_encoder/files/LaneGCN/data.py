...
class ArgoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if 'preprocess' in self.config and self.config['preprocess']:
            ...
            if self.train and self.config['rot_aug']:
                new_data = dict()
                for key in ['city', 'orig', 'gt_preds', 'has_preds', 'gt_hists', 'file_name', 'ref_path', 'ego_aug', 'action', 'ego_maneuver','idx']:
                    if key in data:
    ...
    def get_ego_augmentation(self, data):
        ...
        vel_list_end_y = []
        for i in range(5):
            vel_list_prev_x.append((data['gt_hists'][0][-i - 1, 0] - data['gt_hists'][0][-i - 2, 0]) / 0.1)
            vel_list_prev_y.append((data['gt_hists'][0][-i - 1, 1] - data['gt_hists'][0][-i - 2, 1]) / 0.1)

            vel_list_next_x.append((data['gt_preds'][0][i + 1, 0] - data['gt_preds'][0][i, 0]) / 0.1)
            vel_list_next_y.append((data['gt_preds'][0][i + 1, 1] - data['gt_preds'][0][i, 1]) / 0.1)

            vel_list_end_x.append((data['gt_preds'][0][-i - 1, 0] - data['gt_preds'][0][-i - 2, 0]) / 0.1)
            vel_list_end_y.append((data['gt_preds'][0][-i - 1, 1] - data['gt_preds'][0][-i - 2, 1]) / 0.1)
    ...
    def get_reaction_maneuver_class(self, data):
        ...
        path_cands = self.am.get_candidate_centerlines_for_traj(hist_traj, data['city'], viz=False)
        ...
        closest_lane_obj, conf, dense_centerline, nearby_lane_ids, per_lane_dists = self.am.get_nearest_centerline(hist_traj[0], data['city'], visualize=False)
        ...
        fut_traj = sur_futs[i]
        path_cands = self.am.get_candidate_centerlines_for_traj(fut_traj, data['city'], viz=False)
        ...
        closest_lane_obj, conf, dense_centerline, nearby_lane_ids, per_lane_dists = self.am.get_nearest_centerline(fut_traj[-1], data['city'], visualize=False)
        lane_scores = sorted(per_lane_dists)
        for j in range(len(lane_scores)):
            idx = np.where(per_lane_dists == lane_scores[j])[0][0]
            lane_id = nearby_lane_ids[idx]
