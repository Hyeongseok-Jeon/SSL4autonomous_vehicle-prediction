...
import random
import sys
sys.path.append('/home/jhs/Desktop/SSL4autonomous_vehicle-prediction/LaneGCN/')
import time
from importlib import import_module
...
model = import_module('SSL_encoder')
config, config_enc, Dataset, collate_fn, net, loss, opt = model.get_model('LaneGCN.lanegcn')

avl = ArgoverseForecastingLoader(config['train_split'])
avl.seq_list = sorted(avl.seq_list)
am = ArgoverseMap()

err_file = torch.load('error_data.pk')

err_idx = []
for i in range(len(avl.seq_list)):
    file_name = avl[i].current_seq
    file_name = os.path.basename(file_name).split('/')[0]
    if file_name in err_file:
        err_idx.append(i)
...
def train(config):
    ...
    stores = [None for x in range(len(err_idx))]
    t = time.time()
    for tlqkf in range(len(err_idx)):
        i = err_idx[tlqkf]
    ...
    modify(config, data_loader, 'error_data.p')
