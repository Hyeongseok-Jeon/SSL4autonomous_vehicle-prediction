...
config_action_emb["num_channels"] = [8, 64, 128, 128]
config_action_emb["kernel_size"] = 2
config_action_emb["dropout"] = 0.2
config_action_emb["n_hid"] = 2
...
class encoder(nn.Module):
    ...
    def forward(self, actors, data, init_pred):
        ...
        if init_pred['reg'][0].shape[1] == 6:
            max_idx = [torch.argmax(init_pred['cls'][i]) for i in range(batch_num)]
            init_pred_reg = torch.cat([init_pred['reg'][i][:, max_idx[i], :, :] for i in range(batch_num)])
        else:
