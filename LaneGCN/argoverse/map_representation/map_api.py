...
MAX_LABEL_DIST_TO_LANE = 70  # meters
...
class ArgoverseMap:
    ...
    def get_nearest_centerline(
        ...
        lane lengths differ enormously, meaning the centroid is not indicative of nearby points.fge
        ...
        if visualize:
            ...
            plt.show()
            plt.close("all")
        return closest_lane_obj, conf, dense_centerline, nearby_lane_ids, per_lane_dists

    def get_lane_direction_by_query(
        self, query_xy_city_coords: np.ndarray, city_name: str, visualize: bool = False
    ) -> Tuple[np.ndarray, float]:
        init_idx = 0
        seq_list = self.get_candidate_centerlines_for_traj(query_xy_city_coords[init_idx:], city_name)[1]
        while len(seq_list)==0:
            init_idx = init_idx + 1
            seq_list = self.get_candidate_centerlines_for_traj(query_xy_city_coords[init_idx:], city_name)[1]
        seq_list = sum(seq_list, [])

        cache = self.get_nearest_centerline(query_xy_city_coords[-1,:], city_name)
        lane_obj, confidence, dense_centerline, nearby_lane_ids, per_lane_dists = cache
        closest_sort = np.argsort(per_lane_dists)
        for i in range(closest_sort.shape[0]):
            lane_cand = nearby_lane_ids[closest_sort[i]]
            if lane_cand in seq_list:
                closest_lane = lane_cand
                conf = 1.0 - (per_lane_dists[i]/ OUT_OF_RANGE_LANE_DIST_THRESHOLD)
                conf = max(0.0, conf)  # clip to ensure positive value
                break
        cls = self.get_cl_from_lane_seq([[closest_lane]], city_name)
        centerline = interp_arc(50, cls[0][:,0],cls[0][:,1])

        waypoint_dists = np.linalg.norm(centerline - query_xy_city_coords[-1,:], axis=1)
        ...
        if visualize:
            ...
            plt.scatter(
                query_xy_city_coords[-1,0],
                query_xy_city_coords[-1,1],
                200,
                marker=".",
                color="b",
            )
    ...
    def get_lane_direction(
        ...
        lane_obj, confidence, dense_centerline, trash_1, trahs_2s = cache
    ...
    def get_candidate_centerlines_for_traj(
        ...
        for lane in curr_lane_candidates:
            ...
            candidates_past = self.dfs(lane, city_name, 0, 50, True)
        ...
        candidate_centerlines, indexes = filter_candidate_centerlines(xy, candidate_cl)
        ...
        return candidate_centerlines, obs_pred_lanes
